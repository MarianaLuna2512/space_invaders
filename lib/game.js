...
// invader formation configuration. changing the row/col counts here
// will automatically affect the number of enemies spawned each level.
Game.INVADER_ROWS = 3;          // originally 5
Game.INVADER_COLS = 8;          // originally 13
Game.INVADER_H_SPACING = 35;    // horizontal gap between invaders
Game.INVADER_V_SPACING = 40;    // vertical gap between rows
...
Game.prototype.addInvaderShips = function(level = 1) {
  ...
  // build a grid of invaders using our configuration constants
  for (let row = 0; row < Game.INVADER_ROWS; row++) {
    // choose a sprite name based only on row index; this approach
    // scales with fewer/more rows and guarantees at least one of each
    if (row === 0) {
    ...
    for (let x = 1; x <= Game.INVADER_COLS; x++, invaderIdx++) {
